...
void UkmDatabaseClient::PreProfileInit() {
  if (ukm_recorder_for_testing_) {
    ...
    // Chromium public builds can legitimately run without a UkmService. The
    // upstream Chromium 105 code passed nullptr to UkmObserver here, which
    // crashes while it tries to lock the recorder during browser startup.
    auto* metrics_services_manager =
        g_browser_process->GetMetricsServicesManager();
    auto* ukm_service = metrics_services_manager
                            ? metrics_services_manager->GetUkmService()
                            : nullptr;
    if (!ukm_service)
      return;
